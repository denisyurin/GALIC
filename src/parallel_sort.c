...
static void get_local_rank(char *element,
			   ...
			   long long left, long long right, size_t * loc, int (*compar) (const void *,
											 const void *));
...
static void check_local_rank(char *element,
			     ...
			     size_t noffs_thistask, long long left, long long right, size_t loc,
...
{
  return (*comparfunc) (median_element_list + *((int *) a) * element_size,
			median_element_list + *((int *) b) * element_size);
}
...
double parallel_sort_comm(void *base, size_t nmemb, size_t size, int (*compar) (const void *, const void *),
			  MPI_Comm comm)
{
  ...
  if(Local_NTask > 1 && Color == 1)
	...
	noffs[i] = noffs[i - 1] + nlist[i - 1];
      ...
      size_t *source_tie_braking_rank_list =
	mymalloc("source_tie_braking_rank_list", Local_NTask * sizeof(long long));
      ...
      if(range_len >= 1)
	{
      ...
      MPI_Gather(&range_len, sizeof(long long), MPI_BYTE, range_len_list, sizeof(long long), MPI_BYTE, 0,
		 MPI_CommLocal);
      MPI_Gather(median_element, size, MPI_BYTE, median_element_list, size, MPI_BYTE, 0, MPI_CommLocal);
      MPI_Gather(&tie_braking_rank, sizeof(size_t), MPI_BYTE, tie_braking_rank_list, sizeof(size_t), MPI_BYTE,
		 0, MPI_CommLocal);
      ...
      do
	{
	  for(i = 0; i < Local_NTask - 1; i++)
	      ...
	      if(current_glob_rank[i] != desired_glob_rank[i])
		{
		  get_local_rank(element_guess + i * size, element_tie_braking_rank[i],
				 (char *) base, nmemb, size, noffs[Local_ThisTask], range_left[i],
				 range_right[i], &current_loc_rank[i], compar);
	  ...
	  MPI_Alltoall(current_loc_rank, sizeof(size_t), MPI_BYTE, list, sizeof(size_t), MPI_BYTE,
		       MPI_CommLocal);
	  for(j = 0, rank = 0; j < Local_NTask; j++)
	    rank += list[j];
	  MPI_Allgather(&rank, sizeof(size_t), MPI_BYTE, current_glob_rank, sizeof(size_t), MPI_BYTE,
			MPI_CommLocal);
	  ...
	  MPI_Alltoall(source_range_len_list, sizeof(long long), MPI_BYTE, range_len_list, sizeof(long long),
		       MPI_BYTE, MPI_CommLocal);
	  MPI_Alltoall(source_median_element_list, size, MPI_BYTE, median_element_list, size, MPI_BYTE,
		       MPI_CommLocal);
	  MPI_Alltoall(source_tie_braking_rank_list, sizeof(size_t), MPI_BYTE, tie_braking_rank_list,
		       sizeof(size_t), MPI_BYTE, MPI_CommLocal);
	      ...
	      if(current_glob_rank[Local_ThisTask] != desired_glob_rank[Local_ThisTask])	/* in this case we're not yet done for this split point */
		{
		  ...
		  for(j = 0; j < nleft; j++)
		      ...
		      if(range_len_list[j] < 1)
			{
			  ...
			  if(range_len_list[nleft - 1] >= 1 && j != (nleft - 1))
			    {
			      memcpy(median_element_list + j * size, median_element_list + (nleft - 1) * size,
				     size);
			      memcpy(tie_braking_rank_list + j, tie_braking_rank_list + (nleft - 1),
				     sizeof(size_t));
			      max_loc_list[j] = max_loc_list[nleft - 1];
			    }
	  ...
	  MPI_Allgather(&new_tie_braking_rank, sizeof(size_t), MPI_BYTE, element_tie_braking_rank,
			sizeof(size_t), MPI_BYTE, MPI_CommLocal);
      ...
      for(j = 0; j < Local_NTask; j++)
	{
      ...
      /* exchange the data */
      MPI_Alltoallv(base, send_count, send_offset, MPI_BYTE,
		    basetmp, recv_count, recv_offset, MPI_BYTE, MPI_CommLocal);
			     ...
			     size_t loc, int (*compar) (const void *, const void *))	/* user-specified  comparison function */
{
  ...
  if(count != loc)
    terminate("Inconsistency: Task=%d: loc=%lld count=%lld  left=%lld right=%lld  nmemb=%lld\n", ThisTask,
	      (long long) loc, count, left, right, (long long) nmemb);
}
#endif
