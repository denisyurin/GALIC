		       ...
		       MPI_Datatype recvtype, int source, int recvtag, MPI_Comm comm, MPI_Status * status)
{
  ...
  for(i = 0, p = buf; i < 512; i++, p++)
	...
	{
	  sprintf
	    (msg, "MPI-ERROR: Task=%d/%s: Recv occured before recv buffer. message-size=%d from %d, i=%d c=%d\n",
  ...
  for(i = 0, p = recvbuf + recvcount * size_recvtype; i < 512; i++, p++)
	...
	{
	  sprintf
	    (msg, "MPI-ERROR: Task=%d/%s: Recv occured after recv buffer. message-size=%d from %d, i=%d c=%d\n",
	     ThisTask, getenv("HOST"), recvcount, dest, i, *p);
	  terminate(msg);
	}
    }
