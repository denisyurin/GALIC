...
/* V. Springel modified some of the memory allocation calls to inline it with
 * our internal memory handler.
 */
...
pqueue_t *pqueue_init(size_t n,
		      pqueue_cmp_pri_f cmppri, pqueue_get_pri_f getpri, pqueue_set_pri_f setpri, pqueue_get_pos_f getpos, pqueue_set_pos_f setpos)
{
  pqueue_t *q;

  q = mymalloc("q", sizeof(pqueue_t));

  /* Need to allocate n+1 elements since element 0 isn't used. */
  q->d = mymalloc("q->d", (n + 1) * sizeof(void *));
...
static void bubble_up(pqueue_t * q, size_t i)
{
  ...
  for(parent_node = parent(i); ((i > 1) && q->cmppri(q->getpri(q->d[parent_node]), moving_pri)); i = parent_node, parent_node = parent(i))
    {
  ...
  if(q->size >= q->avail)
    {
      ...
      q->d = myrealloc(q->d, sizeof(void *) * newsize);
      q->avail = newsize;
    }
...
void pqueue_dump(pqueue_t * q, FILE * out, pqueue_print_entry_f print)
{
  ...
  for(i = 1; i < q->size; i++)
    {
      fprintf(stdout, "%d\t%d\t%d\t%d\t%ul\t", i, left(i), right(i), parent(i), (unsigned int) maxchild(q, i));
      print(out, q->d[i]);
    }
}
