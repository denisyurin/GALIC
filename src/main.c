...
double eval_fit_mod(int n, double vel[3], double *massOrbit_new, double *massOrbit_old, 
	...
	/*
	value_r = calc_stack_difference_mod(updated_EGs_EgyResponse_r, EGs_EgyTarget_r[type], 0, 0, 0, EG_MaxLevel,
												EGs_MassTarget[type], EGs_EgyTarget_r[type],
												All.MinParticlesPerBinForDispersionMeasurement * MType[type] / NType[type], DGs_Distance, 2) / MType[type] ;

	value_t = calc_stack_difference_mod(updated_EGs_EgyResponse_t, EGs_EgyTarget_t[type], 0, 0, 0, EG_MaxLevel,
												EGs_MassTarget[type], EGs_EgyTarget_t[type],
												All.MinParticlesPerBinForDispersionMeasurement * MType[type] / NType[type], DGs_Distance, 2) / MType[type] ;
	
	value_p = calc_stack_difference_mod(updated_EGs_EgyResponse_p, EGs_EgyTarget_p[type], 0, 0, 0, EG_MaxLevel,
												EGs_MassTarget[type], EGs_EgyTarget_p[type],
												All.MinParticlesPerBinForDispersionMeasurement * MType[type] / NType[type], DGs_Distance, 2) / MType[type];

	value_q = calc_stack_difference_mod(updated_EGs_EgyResponse_q, EGs_EgyTarget_q[type], 0, 0, 0, EG_MaxLevel,
												EGs_MassTarget[type], EGs_EgyTarget_q[type],
												All.MinParticlesPerBinForDispersionMeasurement * MType[type] / NType[type], DGs_Distance, 2)/ MType[type] ;
	/**/
	...
	//double value_rtqp_s = fac_value_rs[type]*value_rs + fac_value_ts[type]*value_ts  + fac_value_qs[type]*value_qs + fac_value_ps[type]*value_ps;
	
	
	
	//return value  +  Srelfac[type] * ( 30*(value_rs + value_ts  + value_qs) + (value_r + value_t + value_p + value_q)) / Srelfac_count[type];
	

	//return value  /*+value_rs + value_ps*/; //Srelfac[type] * ( value_rtqp_s + (value_r + value_t + value_p + value_q)) / Srelfac_count[type];
	

		//printf("%6.3e | %17.14e  %17.14e  %17.14e\n", value,  value_rs,  value_ts,  value_ps ); /**/
	/*
	...
	/*
	if(ThisTask == 1 && type == 1) 
		printf("%6.3e | %17.14e  %17.14e\n", 4*value,  
				     ( value_r + value_t + value_p + value_q)/4,  
				 0.25*(fac_value_rs[type]*value_rs + fac_value_ts[type]*value_ts + fac_value_ps[type]*value_ps + fac_value_qs[type]*value_qs)/4
				); /**/
				 
				 
				 
	//return value  + value_rs;
